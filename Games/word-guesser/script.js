"use strict";
{
    ...
    if (!board || !keyboard || !statusElement || !toast || !statsModal || !helpModal || !themeButton || !nextPuzzleButton) {
        throw new Error("Could not find the Wordl Guesser game elements.");
    }
    const STATS_STORAGE_KEY = "word-guesser-stats";
    const THEME_STORAGE_KEY = "word-guesser-theme";
    const KEYBOARD_ROWS = ["QWERTYUIOP", "ASDFGHJKL", "ZXCVBNM"];
    const MAX_GUESSES = 6;
    const WORD_LENGTH = 5;
    const loadGithubWordBank = async () => {
    ...
    const createKeyboardButton = (label, action) => {
